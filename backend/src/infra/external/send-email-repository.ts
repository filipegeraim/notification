...
export class SendEmailRepository implements SendNotificationRepository {
  ...
  async send(notification: NotificationDto, user: UserDto) {
    try {
      // TODO: Create a logic to integrate with a partner to send a email, and after send, save on notification history

      // call function

      //Save Notification History
      await this.historyRepository.create({ notification: notification, user: user, channel: Channel.EMAIL });
    } catch (err) {
      throw new SendError('Occurs an error to process email.');
    }
  }
}
